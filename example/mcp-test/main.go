...
func NewMCPServer() *server.MCPServer {
	...
	mcpServer.AddTool(mcp.NewTool("乘法",
		mcp.WithDescription("乘法"),
		mcp.WithNumber("a",
			mcp.Description("First number"),
			mcp.Required(),
		),
...
func handleMultiplicationTool(
	...
	return &mcp.CallToolResult{
		Content: []mcp.Content{
			mcp.TextContent{
				Type: "text",
				Text: fmt.Sprintf("The product of %f and %f is %f.", a, b, product),
			},
		},
	}, nil
}
...
func main() {
	s := mcp_server.NewServer(
		mcp_server.WithName("mcp-server-sse"),
		mcp_server.WithAddr("127.0.0.1:9001"),
		mcp_server.WithTransport("sse"))

	s.GetHandler().AddTool(mcp.NewTool("乘法",
		mcp.WithDescription("乘法"),
		mcp.WithNumber("a",
			mcp.Description("First number"),
			mcp.Required(),
